...
class LLMInterface:
    ...
    def check_model_available(self) -> bool:
        ...
        try:
            ...
            # Note: The ollama-python library has changed its response structure.
            # This code handles both the newer object-based response and the older
            # dictionary-based response for broader compatibility.
            if hasattr(models_response, 'models'): # New API
                available_models = [m.name for m in models_response.models if hasattr(m, 'name')]
            elif isinstance(models_response, dict) and 'models' in models_response: # Old API
                available_models = [m['name'] for m in models_response['models']]
            else:
                print(f"Unknown response type from Ollama client: {type(models_response)}")
                return False

            # --- IMPROVEMENT ---
            # Perform a more precise check that ignores the model tag (e.g., ':latest').
            # This prevents false positives from substring matches.
            target_model_base = self.model.split(':')[0]
            return any(m.split(':')[0] == target_model_base for m in available_models)

        except Exception as e:
            print(f"Error checking for Ollama models: {e}")
            return False
    ...
    def generate_response(
        ...
        if stream:
            ...
            if hasattr(response, 'message'):
                ...
                raise TypeError(f"Received unknown response type from Ollama chat: {type(response)}")
